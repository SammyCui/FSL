...
def download_pkl_from_link(link, data_root, mode):
    filename = 'mini-imagenet-cache-' + mode
    file_path = os.path.join(data_root, filename)

    if not os.path.exists(file_path + '.pkl'):
        print('Downloading:', file_path + '.pkl')
        urllib.request.urlretrieve(link, file_path + '.pkl')
    else:
        print("Data was already downloaded")
    ...
    def __init__(
        ...
        try:
            if not self._check_exists() and download:
                ...
                download_pkl_from_link(dropbox_file_link, self.root, mode)
...
if __name__ == '__main__':
    mi = MiniImageNet(root='../data/mini-imagenet', mode='validation', download=True)
    print()
